...
| **Browser** | any MCP browser server (stdio or streamable-http) keyword-routed per action; Playwright local fallback; DOM-XSS sink discovery; proxy traffic export → replay templates; **JS-challenge wall solver** (`waf_clear`) — rides out Cloudflare IUAM / Incapsula / Akamai-sensor / DataDome / PerimeterX gates in headless Chromium and transplants the clearance cookies + User-Agent into both the agent client and the probe kit |
